...
# new script:
# python -m torch.distributed.launch --nproc_per_node=1 eval_semisup.py --results "C:\Users\tomer\researchsoftware\furniturestyle\logging.csv"   --data_path "C:\Users\tomer\Datasets" --pretrained checkpoints/swav_800ep_pretrain.pth.tar --labels_perc "100" --num_classes 3 --topk 3 --lr 0.01 --lr_last_layer 0.2 --wd 0.002

# Old script:
# python -m torch.distributed.launch --nproc_per_node=1 eval_semisup.py --data_path "C:\Users\tomer\Datasets" --pretrained checkpoints/swav_800ep_pretrain.pth.tar --labels_perc "100" --lr 0.01 --lr_last_layer 0.2 --num_classes 3
...
def log_results(file_path,
                stats={"train_loss": "NA", "train_acc": "NA", "val_loss": "NA", "val_acc": "NA", "val_recall1": "NA",
                       "val_recallk": "NA"}):
    ...
    with open(file_path, 'a') as f:
        ...
        str_out = str_out.format(model_name="eval_semisup.py",
                                 num_classes=args.num_classes,
                                 train_loss=stats["train_loss"],
                                 train_acc=stats["train_acc"],
                                 val_loss=stats["val_loss"],
                                 val_acc=stats["val_acc"],
                                 val_recall1=stats["val_recall1"],
                                 val_recallk=stats["val_recallk"],
                                 epoch=stats["epoch"],
...
def main(a, logger, training_stats):
    ...
    all_styles = [x[0].split(os.sep)[-1] for x in os.walk(train_data_path)][1:]
    list_imgs = []
    list_labels = []
    for style in all_styles:
        temp_imgs = os.listdir(os.path.join(train_data_path, style))
        temp_imgs = temp_imgs[:int(len(temp_imgs) * int(args.labels_perc) / 100)]
        temp_labels = [style] * len(temp_imgs)
        list_imgs.extend(temp_imgs)
        list_labels.extend(temp_labels)

    train_dataset.samples = [(
        os.path.join(train_data_path, lbl, im),
        train_dataset.class_to_idx[lbl]
    ) for im, lbl in zip(list_imgs, list_labels)]
    ...
    # build model
    model = resnet_models.__dict__[args.arch](output_dim=args.num_classes)

    # convert batch norm layers
    model = nn.SyncBatchNorm.convert_sync_batchnorm(model)

    # print("seti model resent and converted")
...
def train(model, optimizer, loader, epoch):
    ...
    for iter_epoch, (inp, target) in enumerate(loader):
        ...
        if args.rank == 0 and iter_epoch % 50 == 0:
            ...
            # TODO: save the acc to CSV
            # TODO: save the acc to CSV
            # TODO: save the acc to CSV loss.val loss.avg
...
def validate_network(val_loader, model):
    ...
    if top1.avg.item() > best_acc[0]:
        best_acc = (top1.avg.item(), topk.avg.item())
...
def retrieval(val_loader, model):
    ...
    with torch.no_grad():
        ...
        with open(args.dump_path + "/" + 'eval_semisup_embeddings.npy', 'wb') as f:
    ...
    with open(args.dump_path + "/" +'eval_semisup_embeddings_image_paths.csv', 'w') as f:
        for idx1, embedding1 in enumerate(embeddings):
            ...
            similar_images[paths[idx1]] = [(path, dist) for path, dist in
                                           zip(paths[all_sorted_results_idx],
                                               current_distances[all_sorted_results_idx])]
            f.write("%s\n" % (paths[idx1]))
