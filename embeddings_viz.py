...
def get_discrete_position(xs, ys, img_dim, cell_size = 1.3, min_cell_size = 1.0, max_cell_size = 100.0):
    min_xs, max_xs = min(xs), max(xs)
    min_ys, max_ys = min(ys), max(ys)
    width = max_xs - min_xs 
    height = max_ys - min_ys
...
npy_file_ = 'eval_semisup_embeddings.npy'
csv_file_ = 'eval_semisup_embeddings_image_paths.csv'

npy_file_ = 'eval_linear_embeddings_2021-04-12_16-53.npy' #'eval_linear_embeddings.npy'
csv_file_ = 'eval_linear_embeddings_image_paths_2021-04-12_16-53.csv' #'eval_linear_embeddings_image_paths.csv'

prefix = npy_file_.split(".")[0]
save_folder = "visualization"

IMG_DIM = 124 # must divide by 4!!
with open(npy_file_, 'rb') as f:
    ...
    tsne = 	TSNE(n_components=2, random_state=42 ).fit_transform(embeddings_reshaped)
    ...
    with open(csv_file_, 'r') as fi:        
        image_paths = [ line.strip() for line in fi]
        ...
        if True:
            ...
            xs = [e[0] for e in X]
            ys = [e[1] for e in X]            
            img = Image.new("RGB", (max(xs), max(ys)), (255, 255, 255))
        ...
        if False: # general scatter plot - need to actually show classes 
            plt.figure(figsize=(6, 5))
            c = 'r' 
